...
export default class S3BucketService {
  ...
  public static async getDownloadLink(bucket: string, key: string): Promise<string> {
    const options = {
      Bucket: bucket,
      Key: key,
    };
  ...
  public static async getDownloadLinks(bucket: string, prefix?: string): Promise<string[]> {
