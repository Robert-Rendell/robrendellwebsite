...
class TechTestUniDataAPI {
  ...
  private static async grabLatestS3Files(): Promise<any> {
    const institutionsFilePromise = S3BucketService.s3
      .getObject({
    ...
    const submissionsFilePromise = S3BucketService.s3
      .getObject({
    ...
    return Promise.all([institutionsFilePromise, submissionsFilePromise]);
  }

  /**
   * GET the dashboard of graphs
   */
  public static async getDashboardGraphs(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      const s3Files: GetObjectOutput[] =
        await TechTestUniDataAPI.grabLatestS3Files();
      const institutionFile: GetObjectOutput = s3Files[0];
      const submissionsFile: GetObjectOutput = s3Files[1];
      const institutions = JSON.parse(
        institutionFile.Body?.toString() || ""
      ) as Institution[];
      const submissions = JSON.parse(
        submissionsFile.Body?.toString() || ""
      ) as Submission[];

      const analyser = new UniDataAnalyserService(institutions, submissions);

      // Decouple these two methods to allow for flexibility in the use of the data
      const bestUnisForSubject: Graph =
        UniDataAnalyserService.getMeanUnisForSubjectsGraph(
      ...
      const response: GetDashboardGraphsResponse = {
        ...
        bestUnisForSubject,
        submissionsPerYear: analyser.submissionsPerYear(),
